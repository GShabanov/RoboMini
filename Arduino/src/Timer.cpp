...
CTimer::CTimer()
{
    ...
    //TCCR1B |= _BV(CS10); // Set CS10 and CS12 bits for 1024 prescaler:
    //TCCR1B |= _BV(CS12);
    //TCCR1B |= _BV(CS10);   // Set CS10 and CS11 bits for 64 prescaler:
    //TCCR1B |= _BV(CS11);
...
void
CTimer::SetCounter(uint8_t index, uint16_t millis)
{
...
void CTimer::Timer1COMPA(void)
{
    ...
    volatile uint16_t *values = lpThis->m_TimedValues;
